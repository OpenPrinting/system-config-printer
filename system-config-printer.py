...
class NewPrinterGUI(GtkGUI):
    ...
    def get_hpfax_device_id(self, faxuri):
        # Read directly from HPLIP's printer properties database, as
        # "hp-info -d" only works if there is already a queue for the
        # printer
        f = open('/usr/share/hplip/data/models/models.dat', 'r')
        res = re.search ("^hp(fax|)://?[^/]+/([^/?]+)([/?].*|)$", faxuri)
        if res:
            resg = res.groups()
            urimodel = resg[1].lower ()
        else:
            return None
        found = False
        for line in f:
            if line.strip ().startswith ("[") == -1:
                continue
            sectionmodel = line.strip ()[1:-1].lower ()
            if urimodel == sectionmodel:
                found = True
                break
        if not found:
            return None
        ...
        for line in f:
            if line.strip ().startswith ("[") == -1:
                break
            if line.strip ().startswith ("fax-type") == -1:
                continue
            res = re.search ("fax-type=(\d+)", line)
            if res:
                resg = res.groups()
                faxtype = resg[0]
            if faxtype >= 0:
